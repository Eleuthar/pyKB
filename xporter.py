...
def xport(data: dict, fname: str):
    ...
    for header in data.keys():
        header_end_col = header_begin_col + len(data[header].keys())
        
        # headers are on the same row, but different columns
        page.merge_range(f"{chr(header_begin_col)}1:{chr(header_end_col)}1", header, header_format)
        
        # visual separation of the compared tables
        header_begin_col = header_end_col + 1
        bom = data.pop(header)
        ...
        for bom_header, bom_data in bom.items():
            begin_col = chr(header_begin_col + next_header_col)
            page.write(f"{begin_col}{subhead_row}", bom_header, header_format)
            next_header_col += 1

            # first column is index, can support row merging
            data_row = 3
            subhead_list = list(bom_data)
            ndx_column = subhead_list.pop(0)
        
            for pkg in bom_data[ndx_column]:
                pkg_count = bom_data[ndx_column].count(pkg)
                if pkg_count > 1:
                    # add all other fields that do not require row merging
                    for row in range(data_row, data_row + pkg_count):
                        for col in range(ord(begin_col), ord(begin_col + len(subhead_list))):
                            page.write(f"{col}{row}", pkg, regular_format)
                    # merge index field rows
                    page.merge_range(f"{begin_col}{data_row}:{begin_col}{data_row + pkg_count}", pkg, regular_format)
                    data_row += pkg_count + 1
                else:
                    page.write(f"{begin_col}{data_row}", pkg, regular_format)
                    data_row += 1
